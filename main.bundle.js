		...
		if ( parents.weights.style == 'func' ) {			
			...
			switch (parents.weights.style) {
				...
				case 'random': 
					...
					break;
				default:
					throw new Error('need a parents.weights.func.style:\t' + parents.weights.style);
			}
			// we don't have an object to show
...
function randomUniform(sup, size){
	...
	let w = sup[1] - l;
	// let low, high;
	let low 	= [...Array(size)].map(() => sup[0] + w * Math.random() );
	...
	while ( i++ < size-1 ) {
		...
		// while (low[i] > high[i] - sup[2] ) {
		// 	high[i] = l + w * Math.random();
		// }
...
class MultiPressure extends _MultiStat_js__WEBPACK_IMPORTED_MODULE_0__["default"] {
	...
	constructor(cfg, thresholds=null, weights=null) {
		...
		// this.ss = this.solSet.statios.map( s => s.getPath(this) );
		// this.ss = this.getSolSet();
...
"use strict";
module.exports = JSON.parse('{"control-variable":"P","response-variable":"P/V","input":{"value":1,"min":0,"max":1,"step":0.1,"history":[]},"output":{"value":1,"min":0,"max":1},"branches":{"equation-style":"logistic"},"draw":{"io":{"Healthy":true,"Cycle":true,"SolSet":true,"solSetDisabled":false},"pp":{"hysterons":true,"hystegon":true,"support":false,"distribution":{"thresholds":false,"weights":false}}},"thresholds":{"pairs":"lattice","pairsOther":"lattice/continuous","support":{"low":0,"high":1,"diag":0},"lattice":{"size":3},"continuous":{"size":2,"distribution":"normal","distOther":"uniform/linear/normal","normal":{"mean":{"low":0,"high":1},"std":{"low":0.2,"high":0.2,"same":true}},"linear":{"slope":1}}},"weights":{"style":"func","styleOther":"equal/random/func","random":{"style":"uniform","styleOther":"uniform/normal - actually not allowing \'normal\'","normal":{"mean":0.5,"std":0.1}},"func":{"style":"linear","styleOther":"linear/normal","linear":{"slope":1},"normal":{"mean":{"low":0,"high":0},"std":{"low":0.2,"high":0.2,"same":true}}}},"recruitability":{"open":0,"recruitable":1,"closed":0}}');
